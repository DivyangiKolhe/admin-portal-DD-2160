...
const AddExercise = ({ onClose }) => {
  ...
  const [masterExerciseId, setMasterExerciseId] = useState(""); 
    ...
    const fetchMasterExercises = async () => {
      try {
        ...
        const response = await neurorehabAPI.get("/master-exercises", {
          ...
          setMasterExercises(response.data.data.masterExercises);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const newExercise = {
        name,
        masterExerciseId, // Use the selected master yoga ID
        breakThumbnail,
        setsThumbnail,
        thumbnail,
        exerciseVideoLink,
      ...
      const response = await rehabAPI.post("/exercises", newExercise, {
        headers: {
  ...
  return (
    <div className="bg-white p-4 rounded-3xl w-[75vw] lg:w-[50vw] h-[70vh] max-h-[95vh] overflow-auto mx-auto px-10 py-8 relative">
      ...
      <h1 className="text-2xl mb-4 font-bold text-center">Exercise Details</h1>
      <form onSubmit={handleSubmit} className="flex flex-col">
        ...
        <div className="mb-4">
          <label
            htmlFor="masterExerciseId"
            className="block text-gray-700 text-sm font-medium"
          >
            Select Exercise Master
          </label>
          <select
            id="masterExerciseId"
            name="masterExerciseId"
            value={masterExerciseId}
            onChange={(e) => setMasterExerciseId(e.target.value)}
            className="w-full border border-gray-300 rounded p-2"
            required
          >
            <option value="">Select Exercise Master</option>
            {masterExercises.map((masterExercise) => (
              <option key={masterExercise.id} value={masterExercise.id}>
                {masterExercise.name}
              </option>
            ))}
          </select>
        ...
        <div className="mb-4">
          ...
          <input
            type="text"
            id="exerciseVideoLink"
            name="exerciseVideoLink"
            value={exerciseVideoLink}
            onChange={(e) => setExerciseVideoLink(e.target.value)}
            className="w-full border border-gray-300 rounded p-2"
            required
          />
        </div>
