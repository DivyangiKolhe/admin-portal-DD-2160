...
import { Card, IconButton, Modal, Snackbar, selectClasses } from '@mui/material';
...
const Blogs = () => {
        ...
        const fetchData = async () => {
            try {
                ...
                const response = await api.get(`/blogs`, {
                    ...
                    params: { blogType: "meditation-routines" },
                });
    ...
    return (
        <div className='flex flex-col gap-4 px-8 py-4 border rounded-lg bg-purple-100'>
            <div className='title text-lg text-purple-900 font-extrabold '>Blogs</div>
                ...
                {blogs.length > 0 ? (
                    blogs.map((item) => (
                        <div key={item.id}
                            ...
                            <div className='h-[6rem] rounded-lg bg-center bg-cover' style={{ backgroundImage: `url(${item.imageUrl})`, backgroundSize: "cover" }} />
            ...
            {selectedBlog && (
                <Modal open={openViewModal} onClose={handleCloseViewModal} className='flex items-center justify-center'>
                    <div>
                        <ViewBlog blog={selectedBlog} onDelete={handleDeleteBlog} onClose={handleCloseViewModal} />
                    </div>
                </Modal>
            )}
            {/* Add Blog Modal */}
            <Modal open={openModal} onClose={handleCloseModal} className='flex items-center justify-center'>
                <div>
                    <AddBlogs addBlog={addNewBlog} onClose={handleCloseModal} />
                </div>
            </Modal>
            {/* Snackbar */}
            <Snackbar
