...
import { AiOutlineEdit, AiOutlineArrowLeft, AiOutlineClose, AiOutlineSave } from "react-icons/ai";
...
const EditPlaylist = ({ playlist, onClose }) => {
    ...
    const handleSave = async () => {
        try {
            ...
            const { id, isMeditationRoutine, ...editedPlaylistWithoutId } = editedPlaylist;
            ...
            const response = await rehabAPI.patch(
                `/meditation-routine/playlist/${editedPlaylist.id}`,
    ...
    const handleFileChange = async (files) => {
        ...
        try {
            ...
            const url = await uploadToS3({ file: selectedFile, fileType: "image", featureName: "music-therapy" });
            ...
            {isEditing ? (
                <div className="flex gap-2 p-4 justify-between items-center flex-col">
                    ...
                    <div className="flex gap-4">
                        ...
                        <div className="">
                            <label className="block text-gray-700 text-sm font-bold mb-2">Total Duration</label>
                            <input
                                type="text"
                                value={editedPlaylist.totalDuration}
                                className="text-black w-full my-2 p-2 rounded border border-black"
                                onChange={(e) => setEditedPlaylist({ ...editedPlaylist, totalDuration: e.target.value })}
                    ...
                    <div className="flex gap-4">
                        <div className="">
                            ...
                            <input
                                ...
                                className="text-black w-full my-2 p-2 rounded border border-black"
                                onChange={(e) => setEditedPlaylist({ ...editedPlaylist, language: e.target.value })}
                        ...
                        <div className="">
                            ...
                            <input
                                ...
                                className="text-black w-full my-2 p-2 rounded border border-black"
                                onChange={(e) => setEditedPlaylist({ ...editedPlaylist, year: e.target.value })}
                                placeholder="Year"
                            />
                        </div>
                    </div>
                    <div className="flex gap-4">
                        <div className="">
                            ...
                            <input
                                ...
                                className="text-black w-full my-2 p-2 rounded border border-black"
                                onChange={(e) => setEditedPlaylist({ ...editedPlaylist, genre: e.target.value })}
                                placeholder="Genre"
                            />
                        </div>
                    </div>
                    <div className="border border-gray-300 p-2 rounded w-2/3">
                        <label className="block text-gray-700 text-sm font-bold mb-2">Thumbnail Link</label>
                        <input
                            type="text"
                            value={editedPlaylist.thumbnailLink}
                            className="text-black w-full my-2 p-2 rounded border border-black"
                            onChange={(e) => setEditedPlaylist({ ...editedPlaylist, thumbnailLink: e.target.value })}
                ...
                <div>
                    {/* Render view mode of the playlist */}
                </div>
            )}
            <Snackbar
                open={snackbarOpen}
