...
const Playlists = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await rehabAPI.get("/meditation-routine/playlist", {
        headers: {
  ...
  const handleOpenViewModal = async (playlist) => {
    ...
    try {
      ...
      const response = await rehabAPI.get(
        `/meditation-routine/playlist/${playlist.id}`,
        {
          headers: {
            Authorization: `Bearer ${authToken}`,
          },
  ...
  return (
    <div className="flex flex-col gap-4 px-8 py-4 border rounded-lg bg-purple-100">
      ...
      {selectedPlaylist && (
        ...
        >
          <div>
            <ViewPlaylist music={selectedPlaylist} songs={playlistSongs} videos={playlistVideos}/>
          </div>
        </Modal>
      )}
      <Modal open={openAddModal} onClose={handleCloseAddModal}>
        <div> <AddPlaylist onClose={handleCloseAddModal} /></div>
