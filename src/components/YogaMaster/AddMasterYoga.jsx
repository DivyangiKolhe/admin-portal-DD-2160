...
const AddYogaMaster = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await rehabAPI.post("/yoga/master-yoga", newMasterYoga, {
        headers: {
      ...
      if (response.status === 200) {
        setSnackbarMessage("Master Yoga added successfully");
        setSnackbarSeverity("success");
        setSnackbarOpen(true);
        onClose();
      }
    } catch (error) {
      console.error("Failed to add master yoga", error);
      setSnackbarMessage("Failed to add master yoga");
  ...
  const handleFileChange = async (files) => {
    ...
    try {
      ...
      const fileObject = {
        file: selectedFile,
        fileType: "image",
        featureName: "master-yoga",
        filePath:"yoga/images",
      };
  ...
  return (
    <div className="bg-white p-4 rounded-3xl w-[75vw] lg:w-[50vw] h-[70vh] max-h-[95vh] overflow-auto mx-auto px-10 py-8 relative">
      ...
      <h1 className="text-2xl mb-4 font-bold text-center">Add Master Yoga</h1>
