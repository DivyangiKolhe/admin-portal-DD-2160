...
import EditYogaMasterModal from "./EditYogaMasterModal";
import {
  Modal,
  Button,
  Dialog,
...
const MasterYogaCard = ({ data }) => {
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      const response = await rehabAPI.delete(`/yoga/master-yoga/${data.id}`, {
        headers: { Authorization: `Bearer ${authToken}` },
      });
      if (response.status === 200) {
        setSnackbarMessage("Master Yoga deleted successfully");
        setSnackbarOpen(true);
        setIsDeleteModalOpen(false);
        // Optionally, you can perform any additional cleanup or UI updates here.
      } else {
        setSnackbarMessage("Failed to delete Master Yoga");
        setSnackbarOpen(true);
      }
    } catch (error) {
      console.error("Error deleting Master Yoga:", error);
      setSnackbarMessage("Failed to delete Master Yoga");
      setSnackbarOpen(true);
    }
  };
